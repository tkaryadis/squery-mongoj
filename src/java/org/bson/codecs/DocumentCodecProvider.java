...
{
    //public boolean isJavaDecode=true;
    ...
    public <T> Codec<T> get(final Class<T> clazz,final CodecRegistry registry)
    {
        ...
        {
            //TODO,IF POSSIBLE here and not childCoderegistry add try/catch
            if(!((org.bson.internal.ChildCodecRegistry) registry).javaRegistry)
            {
                DocumentCodec cljDocumentCodec=new DocumentCodec(registry, bsonTypeClassMap, valueTransformer);
                cljDocumentCodec.setJavaDecode(false);
                return (Codec<T>) cljDocumentCodec;
            }
            else return (Codec<T>) new DocumentCodec(registry, bsonTypeClassMap, valueTransformer);
        }


        /*
        if (clazz == CljDocument.class) {   //////////////////////////////////////////////////////////////////////////////
            System.out.println("called");
            if(registry instanceof ProvidersCodecRegistry)
            {
                System.out.println("java"+ ((ProvidersCodecRegistry) registry).javaRegistry);
            }
            //System.out.println("ooo"+registry.get(CljDocument.class));
            DocumentCodec cljDocumentCodec=new DocumentCodec(registry, bsonTypeClassMap, valueTransformer);
            cljDocumentCodec.setJavaDecode(2);
            return (Codec<T>) cljDocumentCodec;
        }
